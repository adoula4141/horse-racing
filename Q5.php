...
$outputFile = "data/bets/$raceDate"."SetQ5.php";
...
function getdata($raceDate, $totalRaces, $outputFile, $jockeyNamesAllRaces)
{
    ...
    for ($raceNumber = 1; $raceNumber <= $totalRaces; $raceNumber++) { 
        ...
        $qpl5 = [];
        if(isset($list1[0]) && !in_array($list1[0], $qpl5)) $qpl5[] = $list1[0];
        if(isset($list2[2]) && !in_array($list2[2], $qpl5)) $qpl5[] = $list2[2];

        if(count($qpl5) < 2) $qpl5 = [];
        ...
        $betting .= "\t\t'QUINELLA PLACE' => [" . implode(", ", $qpl5) . "],\n";
        $betting .= "\t\t'QUINELLA' => [" . implode(", ", $qpl5) . "],\n";
