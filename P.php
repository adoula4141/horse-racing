...
$outputFile = "data/bets/$raceDate"."SetP.php";
...
function getdata($raceDate, $totalRaces, $outputFile, $jockeyNamesAllRaces)
{
    ...
    $selection = array_slice($list, 0, 6);
    ...
    $selection = array_values(array_unique(array_merge(
            array_diff($selection, $dList),
            array_diff($dList, $selection)
    )));    
    ...
    for ($raceNumber = 1; $raceNumber <= $totalRaces; $raceNumber++) { 
        ...
        $betting .= "\t\t'WIN' => [" . implode(", ", $selection) . "],\n";
        $betting .= "\t\t'PLACE' => [" . implode(", ", $selection) . "],\n";
